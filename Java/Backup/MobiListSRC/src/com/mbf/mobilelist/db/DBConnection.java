...
public class DBConnection implements java.io.Serializable {
        ...
        private static String server = "10.252.20.67:1521";
        private static String username = "score24";
        private static String password = "score24";
        ...
        private CheckDBConnection cdb = null;
        ...
        public static void main(String[] args)
        {
                try
                {
                        ...
                        System.out.println("Currentid:"+ms.currentId);
                    java.sql.ResultSet rs = ms.stmt.executeQuery("select *  FROM score24.mbf_mobile WHERE group_id =204766");
                        if(rs.next())
                                System.out.println("ac:"+rs.getString(1));
                        rs.close();
                        ms.putConnection();
                } catch(Exception e) {
                        e.printStackTrace();
        ...
        private boolean makeConnection(){
                try {
                        ...
                        //if (inuse>50)
                                System.out.println("DBConn LiveScore: total="+max+" created:"+total+" inuse:"+inuse);
                        if (cdb==null) cdb = new CheckDBConnection();
                        cdb.start();
                        ...
                        for (int i=0; i<poolsize; i++){//Tim kiem cac connection chua su dung
                          if (consState[i]==3){//Trang thai closing
                                  closing++;	
                                  closingCons += ","+i;			
                          } else if (consState[i]!=1){//Truong hop chua su dung co state khac 1
                                        ...
                                        if (cons[i]!=null){
                                                ...
                                                try {
                                                        ...
                                                        //Class.forName("oracle.jdbc.driver.OracleDriver").newInstance();
                                                        //cons[i] = DriverManager.getConnection("jdbc:oracle:thin:@"+server+":loyal2",username,password);
                                                        createConnection(i);
                                                        this.con = cons[i];						
                                                        this.currentId = i;
