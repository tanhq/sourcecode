...
public class DBConnectionManager {
    ...
    private void init() {        
        ...
        try {
            dbProps.load(new FileInputStream("G:\\Services\\Pool\\properties\\db.properties"));
        }
        catch (Exception e) {
    ...
    public static void main(String[] args)
    {
    	...
    	try
    	{
    		DBConnectionManager client = DBConnectionManager.getInstance();
    		for(int i=0;i<100;i++)
    		{    		
    			System.out.println("Thuc hien lan thu "+(i+1));
	    		Connection con = client.getConnection("access");
	    		if (con == null) 
	    		{
	        	...
	        	try 
	        	{
		            ...
		            rs = stmt.executeQuery("SELECT * FROM LEAGUES");
		            while (rs.next()) 
		            {
		        ...
		        client.freeConnection("access", con);
		    }    
	        //client.release();	       	        
    	}
    	catch(Exception ex)
